...
%
% RULES:
%   - Every number traces to results/vit/ or docs/vit/ ; no invention.
%   - macro-F1 is the headline metric; accuracy is supporting only (VLD-10).
%   - Literature comparisons that use a different protocol are CONTEXTUAL only.
%   - No state-of-the-art claims. Classifier is MLP only.
% =====================================================================
...
fark, dengesizliğin doğrudan kanıtıdır: çok düşük destekli nadir sınıflar
makro-F1'i aşağı çeker. Literatür sonuçları, protokol farkları nedeniyle yalnızca
bağlamsal olarak ele alınmış; güncel-en-iyi (state-of-the-art) bir başarı
