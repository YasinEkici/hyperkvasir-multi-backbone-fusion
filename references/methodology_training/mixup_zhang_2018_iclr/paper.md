...
$$R_{\delta}(f) = \int \ell(f(x), y) dP_{\delta}(x, y) = \frac{1}{n} \sum_{i=1}^{n} \ell(f(x_i), y_i). \tag{1}$$
...
We conduct additional image classification experiments on the CIFAR-10 and CIFAR-100 datasets to further evaluate the generalization performance of *mixup*. In particular, we compare ERM and *mixup* training for: PreAct ResNet-18 [\(\hat{He} et al., 2016\)](#page-11-12) as implemented in [\(\hat{Liu}, 2017\)](#page-11-13), WideResNet-28-10 [\(Zagoruyko & Komodakis, 2016a\)](#page-12-2) as implemented in [\(Zagoruyko & Komodakis, 2016b\)](#page-12-3), and DenseNet [\(\hat{Huang} et al., 2017\)](#page-11-14) as implemented in [\(\hat{Veit}, 2017\)](#page-12-4). For DenseNet, we change the growth rate to 40 to follow the DenseNet-BC-190 specification from [\(\hat{Huang} et al., 2017\)](#page-11-14). For *mixup*, we fix \(\alpha = 1\), which results in interpolations \(\lambda\) uniformly distributed between zero and one. All models are trained on a single Nvidia Tesla P100 GPU using PyTorch[2](#page-4-3) for 200 epochs on the training set with 128 examples per minibatch, and evaluated on the test set. Learning rates start at 0.1 and are divided by 10 after 100 and 150 epochs for all models except WideResNet. For WideResNet, we follow [\(\hat{Zagoruyko} & \hat{Komodakis}, 2016a\)](#page-12-2) and divide the learning rate by 10 after 60, 120 and 180 epochs. Weight decay is set to \(10^{-4}\). We do not use dropout in these experiments.
...
|--------|--------|--------------------------|--------------------------|
| LeNet  | ERM    | 9.8<br>10.1<br>10.2      | 10.3<br>10.8<br>11.3     |
| VGG-11 | ERM    | 5.0<br>4.0<br><b>3.9</b> | 4.6<br>3.8<br><b>3.4</b> |
...
|------------------------|--------------------------------|----------|----------|----------------------|--------------------|
| Wethou                 | Specification                  | Input    | Target   | $\overline{10^{-4}}$ | $5 \times 10^{-4}$ |
| ERM                    |                                | Х        | Х        | 5.53                 | 5.18               |
| тіхир                  | AC + RP                        | 1        | <b>✓</b> | 4.24                 | 4.68               |
| •                      | AC + KNN                       | 1        | ✓        | 4.98                 | 5.26               |
| mix labels and latent  | Layer 1                        | 1        | <b>✓</b> | 4.44                 | 4.51               |
| representations        | Layer 2                        | ✓        | ✓        | 4.56                 | 4.61               |
| (AC + RP)              | Layer 3                        | ✓        | ✓        | 5.39                 | 5.55               |
|                        | Layer 4                        | ✓        | 1        | 5.95                 | 5.43               |
|                        | Layer 5                        | 1        | ✓        | 5.39                 | 5.15               |
| mix inputs only        | SC + KNN (Chawla et al., 2002) | <b>√</b> | Х        | 5.45                 | 5.52               |
|                        | AC + KNN                       | ✓        | X        | 5.43                 | 5.48               |
|                        | SC + RP                        | ✓        | X        | 5.23                 | 5.55               |
|                        | AC + RP                        | 1        | X        | 5.17                 | 5.72               |
| label smoothing        | $\epsilon = 0.05$              | Х        | 1        | 5.25                 | 5.02               |
| (Szegedy et al., 2016) | $\epsilon = 0.1$               | X        | ✓        | 5.33                 | 5.17               |
|                        | $\epsilon = 0.2$               | X        | ✓        | 5.34                 | 5.06               |
| mix inputs +           | $\epsilon = 0.05$              | 1        | 1        | 5.02                 | 5.40               |
| label smoothing        | $\epsilon = 0.1$               | ✓        | 1        | 5.08                 | 5.09               |
| (AC + RP)              | $\epsilon = 0.2$               | ✓        | ✓        | 4.98                 | 5.06               |
|                        | $\epsilon = 0.4$               | 1        | ✓        | 5.25                 | 5.39               |
| add Gaussian noise     | $\sigma = 0.05$                | 1        | Х        | 5.53                 | 5.04               |
| to inputs              | $\sigma = 0.1$                 | ✓        | X        | 6.41                 | 5.86               |
|                        | $\sigma = 0.2$                 | ✓        | X        | 7.16                 | 7.24               |

Table 5: Results of the ablation studies on the CIFAR-10 dataset. Reported are the median test errors of the last 10 epochs. A tick ( $\checkmark$ ) means the component is different from standard ERM training, whereas a cross ( $\checkmark$ ) means it follows the standard training practice. AC: mix between all classes. SC: mix within the same class. RP: mix between random pairs. KNN: mix between k-nearest neighbors (k=200). Please refer to the text for details about the experiments and interpretations.
...
- P. Goyal, P. Dollar, R. Girshick, P. Noordhuis, L. Wesolowski, A. Kyrola, A. Tulloch, Y. Jia, and K. He. Accurate, ´ large minibatch SGD: Training ImageNet in 1 hour. *arXiv*, 2017.
...
- G. Pereyra, G. Tucker, J. Chorowski, Ł. Kaiser, and G. Hinton. Regularizing neural networks by penalizing confident output distributions. *ICLR Workshops*, 2017.
...
- P. Simard, Y. LeCun, J. Denker, and B. Victorri. Transformation invariance in pattern recognitiontangent distance and tangent propagation. *Neural networks: tricks of the trade*, 1998.
...
- P. Warden, 2017. URL [https://research.googleblog.com/2017/08/](https://research.googleblog.com/2017/08/launching-speech-commands-dataset.html) [launching-speech-commands-dataset.html](https://research.googleblog.com/2017/08/launching-speech-commands-dataset.html).
- S. Xie, R. Girshick, P. Dollar, Z. Tu, and K. He. Aggregated residual transformations for deep neural networks. ´ *CVPR*, 2016.
- S. Zagoruyko and N. Komodakis. Wide residual networks. *BMVC*, 2016a.
- S. Zagoruyko and N. Komodakis, 2016b. URL [https://github.com/szagoruyko/](https://github.com/szagoruyko/wide-residual-networks) [wide-residual-networks](https://github.com/szagoruyko/wide-residual-networks).
